def test():
    ...
    short_dir = '../../SONY_Test_Data/short/'
    enh_dir = 'restored/'
    ...
    model_bsub5 = torch.load('checkpoints/model_bsub5.pth')
    model_bsub5.eval()
    model_bsub5 = model_bsub5.cuda(device)
    ...
    for i in range(len(filenames)):
        ...
        g2, g3, g4, g5, g6 = BuildGpyr(im_short)
        ...
        with torch.no_grad():
            ...
            s4e = model_bsub4(g4)
            s5e = model_bsub5(g5)
            grad_out = model_grad(g6)
            lpe = modellp(torch.cat((g6, grad_out), dim=1))
    
        im_enh = reconLpyr(s1e, s2e, s3e, s4e, s5e, lpe)
